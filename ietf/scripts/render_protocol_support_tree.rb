...
def schema_children(source, publication_path)
  source.children.select { |path| path.file? && path.basename.to_s.end_with?(".schema.json") }.sort.map do |path|
    ...
    title = schema["title"]
    abort "#{path}: title is required" unless title.is_a?(String) && !title.empty?

    name = path.basename.to_s
    item(title, [[name, "#{LOCAL_PATH}/#{publication_path}/#{name}", false]])
  end
end
...
sections << item(
  "Protocol Schemas",
  [["browse", "#{LOCAL_PATH}/schemas/", false]],
  schema_children(IETF_ROOT.join("schemas"), "schemas")
)
sections << item(
  "Conformance",
  [["browse", "#{LOCAL_PATH}/conformance/", false]],
...
sections << item("Test Vectors", [["browse", "#{LOCAL_PATH}/test-vectors/", false]], test_vector_children)
...
sections << item("Examples", [["browse", "#{LOCAL_PATH}/examples/", false]], example_children)
sections << item("Implementation Guides", [["browse", "#{LOCAL_PATH}/guides/", false]])

problem_children = problem_definitions.map do |code, meaning|
  slug = code.downcase.tr("_", "-")
  item(meaning, [[code, "#{LOCAL_PATH}/problems/#{slug}/", false]])
end
sections << item("Problem Types", [["browse", "#{LOCAL_PATH}/problems/", false]], problem_children)
